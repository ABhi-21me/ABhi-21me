<!-- ================= HEADER ================= -->
<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0D1117&height=180&section=header&text=Abhishek%20Vishwakarma&fontSize=42&fontColor=00D9FF&fontAlignY=65&animation=fadeIn" width="100%" />

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=21&duration=2800&pause=900&color=8B5CF6&center=true&vCenter=true&width=750&lines=Cloud+%2F+DevOps+Engineer-in-Training;AWS+%7C+Docker+%7C+Kubernetes+%7C+Linux;Building+AI-assisted+systems;Turning+ideas+into+shippable+projects" />

<br/>

<img src="https://komarev.com/ghpvc/?username=ABhi-21me&style=for-the-badge&color=8B5CF6&label=PROFILE+VIEWS" />
<img src="https://img.shields.io/github/followers/ABhi-21me?label=FOLLOWERS&style=for-the-badge&color=8B5CF6&labelColor=0D1117" />

</div>

<br/>

<!-- ================= TERMINAL HERO ================= -->
<div align="center">
<img src="terminal-hero.svg" alt="Terminal intro animation" width="680" />
</div>

<br/>

<!-- ================= ABOUT ================= -->
## 🧭 About

- 🎓 3rd-year B.Tech CSE @ **GLA University, Mathura** · Class of 2028 — originally from Gorakhpur, UP
- ☁️ Cloud/DevOps-focused engineer-in-training, deepening AWS and Kubernetes fundamentals
- 🤖 Shipping AI-assisted builds on the side — 70% Cloud & DevOps · 30% AI-assisted builder
- 🔭 Currently sharpening Docker → Kubernetes → Linux for placement season
- 🤝 Building with **Team HashHunters**
- 📡 Open to Cloud/DevOps internship opportunities

<!-- ================= TECH ================= -->
## 🧰 Tech Arsenal

### ⚙️ Currently Working With

<p align="left">
<img src="https://skillicons.dev/icons?i=aws,docker,kubernetes,linux,git,github,python,js,html,css,firebase,flutter&perline=6" />
</p>

### 🧠 Currently Learning

<p align="left">
<img src="https://skillicons.dev/icons?i=terraform,jenkins,nginx,bash,java&perline=6" />
</p>

---

<!-- ================= PROJECTS ================= -->
## 🚀 Featured Projects

<table width="100%">
<tr>
<td width="50%" valign="top">

### ☁️ [Cloud_Grid](https://github.com/ABhi-21me/Cloud_Grid)
AI-driven autonomous cloud security platform (CNAPP) for AWS. Pulls CloudWatch logs on an EventBridge schedule, sends them to Amazon Bedrock for threat analysis, then auto-remediates via AWS SSM and alerts through SNS — no human in the loop.

`AWS Lambda` `Bedrock` `EventBridge` `SSM` `SNS` `Express.js` `MySQL` `Vite`

🏆 Built for **AI FutureReady 2026** (Engineering category) · GLA University team project

</td>
<td width="50%" valign="top">

### 🍹 [Sip-Sutra](https://github.com/ABhi-21me/sip-sutra)
Smart self-service liquor dispensing system for bars and venues — secure digital payments, IoT-powered precision dispensing, automated inventory tracking, and real-time analytics in one platform.

`IoT` `Payments` `Real-time Analytics` `Embedded Systems`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🎬 [Reel Manager](https://github.com/ABhi-21me/reel-manager)
Full-stack tool that automatically categorizes and indexes saved Instagram reels into a searchable local library, running entirely on local AI models.

`FastAPI` `Ollama · Llama 3.1` `faster-whisper` `OpenCV` `Tesseract OCR` `SQLite`

</td>
<td width="50%" valign="top">

### ➕ More in progress
Always shipping something new in Cloud/DevOps and AI-assisted tooling — check out my pinned repos for the latest.

</td>
</tr>
</table>

---

<!-- ================= STATS ================= -->
# 📊 GitHub Telemetry

<div align="center">

<a href="https://github.com/ABhi-21me">
<img height="180" src="https://github-readme-stats.vercel.app/api?username=ABhi-21me&show_icons=true&hide_border=true&theme=github_dark&bg_color=0D1117&title_color=8B5CF6&icon_color=8B5CF6&text_color=FFFFFF&count_private=true" />
<img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=ABhi-21me&layout=compact&hide_border=true&theme=github_dark&bg_color=0D1117&title_color=8B5CF6&text_color=FFFFFF&langs_count=8" />
</a>

<br/><br/>

<img src="https://streak-stats.demolab.com?user=ABhi-21me&theme=github-dark-blue&hide_border=true&background=0D1117&ring=8B5CF6&fire=8B5CF6&currStreakLabel=8B5CF6" />

</div>

---

<!-- ================= TROPHIES ================= -->
# 🏆 Trophy Case

<div align="center">
<img src="https://github-profile-trophy.vercel.app/?username=ABhi-21me&theme=algolia&no-frame=true&no-bg=true&margin-w=15&column=7" />
</div>

---

<!-- ================= SNAKE ================= -->
# 🐍 Contribution Matrix

<div align="center">
<img src="https://raw.githubusercontent.com/ABhi-21me/ABhi-21me/output/github-contribution-grid-snake-dark.svg" alt="GitHub contribution snake animation" />
</div>

---

<!-- ================= ACTIVITY ================= -->
# 📈 Activity

<div align="center">
<img src="https://github-readme-activity-graph.vercel.app/graph?username=ABhi-21me&bg_color=0D1117&color=FFFFFF&line=8B5CF6&point=FFFFFF&area=true&hide_border=true" />
</div>

---

<!-- ================= CONNECT ================= -->
## 🤝 Let's Connect

<div align="center">

<a href="https://github.com/ABhi-21me">
<img src="https://img.shields.io/badge/GitHub-ABhi--21me-111111?style=for-the-badge&logo=github" />
</a>
<a href="https://www.linkedin.com/">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin" />
</a>

<br/><br/>

### `☁️ Build. Deploy. Break. Learn. Repeat. 🚀`

<br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=8B5CF6&height=100&section=footer" width="100%" />

</div>
